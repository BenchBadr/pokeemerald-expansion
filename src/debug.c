...
static u32 Debug_CheckToggleFlags(u8 id)
{
    ...
    case DEBUG_FLAGVAR_MENU_ITEM_TOGGLE_POKENAV:
        result = FlagGet(FLAG_SYS_PHONE_GET);
        break;
...
static void DebugAction_FlagsVars_SwitchPokeNav(u8 taskId)
{
    if (FlagGet(FLAG_SYS_PHONE_GET))
        PlaySE(SE_PC_OFF);
    else
        PlaySE(SE_PC_LOGIN);
    FlagToggle(FLAG_SYS_PHONE_GET);
}
...
#define SOUND_LIST_BGM              \
    ...
    X(MUS_LUTECIA) \
    X(MUS_SYCAMORE) \
    X(MUS_POKEMON_XY_WILD_POKEMON)  \
    X(MUS_BATTLE_JACINTHE)          \
    X(MUS_PKMN_SWORD_SHIELD_GYM_BATTLE_NORMAL) \
    X(MUS_XYTRAINERBATTLE) \
    X(MUS_VS_WILD_2) \
    X(MUS_ZA_TRAINER)
